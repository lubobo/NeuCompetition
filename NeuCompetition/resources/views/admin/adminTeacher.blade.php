...
@section('content')
    <div style="margin-top:10px;">
        <div class="col-xs-12 row">
            <form action="{{route('adminTeacher')}}">
                <div class="col-xs-6">
                    <div class="col-xs-5">
            ...
            <div id="operate">
                ...
                <div class="col-xs-0 btn btn-sm btn-info">
                    <span class="glyphicon glyphicon-user"> {{count($teachers).' '.'人'}}</span>
                </div>
            </div>
            <div class="col-xs-12">
                <table class="table text-center" id="table">
                    <tr class="bg-light-blue-active">
                        ...
                        <td class="col-xs-2">学科</td>
                        <td class="col-lg-2">职称</td>
                        <td class="col-lg-1">姓名</td>
                        <td class="col-lg-1">工资号</td>
                    ...
                    @foreach($teachers as $teacher)
                        <tr class="bg-gray-light">
                            <form action="{{route('delete_teacher')}}" method="POST">
                                <td class="col-xs-1">
                                    <input type="checkbox" name="teacher_id" class="each_checkbox" value="{{$teacher->id}}">
                                    {{$teacher->id}}
                                </td>
                                <td class="col-xs-2">{{$teacher->subject}}</td>
                                <td class="col-lg-2">{{$teacher->job_title}}</td>
                                <td class="col-lg-1">{{$teacher->name}}</td>
                                <td class="col-lg-1">{{$teacher->num}}</td>
                                <td class="col-xs-2">
                                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                                    {{--<button type="submit" class="btn btn-xs btn-danger">删除</button>--}}
                                    <a  class="btn btn-xs btn-success" data-toggle="modal" data-target="#myModal">
                                        <span class="glyphicon glyphicon-repeat"> 修改密码</span>
                                    </a>
                                </td>
                            </form>
                            <form class="form-group" action="{{route('change_teacher')}}" method="post">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <h4 class="modal-title" id="myModalLabel">
                                                    <span class="glyphicon glyphicon-repeat">重置密码</span>
                                                </h4>
                                            </div>
